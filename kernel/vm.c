...
//判断一个虚拟地址是不是因为惰性分配还没分配和映射地址, 是的话就返回1
int uvmshouldallocate(uint64 va) {      
    pte_t* pte;
    struct proc* p = myproc();

    return va < p->sz               //确保地址在进程的内存大小范围内
     && PGROUNDDOWN(va) != r_sp()     //确保地址不在 guard page中（这里的sp是指向进程的内核栈的最顶端，虚拟地址从0开始，所以PGROUNDDOWN(va)就是guard page的起始地址）
      && (((pte = walk(p->pagetable, va, 0)) == 0) || ((*pte & PTE_V) == 0)); // 分配了和va一样的页表项地址，但是这个页表项不存在
}
