...
userinit(void)
{
  ...
  p->sz = PGSIZE;
  //同步程序内存映射到进程内核页表中
  kvmcopymappings(p->pagetable, p->kama_kernelpgtbl, 0, p->sz);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    //内核页表中的映射和用户态页表同步扩大
    if (kvmcopymappings(p->pagetable, p->kama_kernelpgtbl, sz, n) != 0)
    {
    ...
    //内核页表中的映射和用户态页表同步缩小
    sz = kvmdealloc(p->kama_kernelpgtbl, sz, sz + n);
  }
  p->sz = sz;
  return 0;
}
...
fork(void)
{
  ...
  // 加入调用kvmcopymappings，将新进程用户页表映射拷贝到新进程内核页表中
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0 || 
    kvmcopymappings(np->pagetable, np->kama_kernelpgtbl, 0, p->sz) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
